#!/usr/bin/env bash
# Wrapper for scripts/calendar_refresh_schedules.py invoked from the
...
# Shared with calendar_sweep_values_wrapper.sh — both jobs write the
# engine DB and a 04:15 sweep can otherwise overlap a still-running
