...
@dataclass(frozen=True)
class BISDataflow:
    """Represents a dataset in the BIS catalog."""
...
@dataclass(frozen=True)
class BISDimension:
    """A single dimension in a BIS data structure definition."""
...
@dataclass(frozen=True)
class BISDataStructure:
    """Full DSD with dimensions for a BIS dataflow."""

    id: str
    version: str
    name: str = ""
    dimensions: tuple[BISDimension, ...] = ()
    dataflow_id: str = ""
    dataflow_version: str = ""


@dataclass(frozen=True)
class BISStructureSummary:
...
@dataclass(frozen=True)
class BISSizeEstimate:
    """Observation count estimate for a BIS dataflow."""
...
class BISAPIError(RuntimeError):
    """Base error for BIS API failures."""


class BISRateLimitError(BISAPIError):
    """Raised when BIS throttles a request (HTTP 429)."""
...
class BISClient:
    ...
    def __init__(self, *, timeout: int = 30) -> None:
        ...
        self._dataflow_cache: list[BISDataflow] | None = None
        self._structure_cache: dict[str, BISDataStructure] = {}
    ...
    def _get(
        ...
        *,
        ...
        if response.status_code == 429:
            ...
            raise BISRateLimitError(f"BIS rate limit exceeded for {url}.{suffix}")
        try:
            ...
            if detail:
                raise BISAPIError(
                    f"BIS request failed for {url}: HTTP {response.status_code}: {detail[:200]}"
                ) from exc
            raise BISAPIError(
                f"BIS request failed for {url}: HTTP {response.status_code}"
            ) from exc
        return response
    ...
    def get_data(
        ...
        *,
        ...
        Args:
            ...
            end_period: Optional end filter.
    ...
    def list_dataflows(self) -> list[BISDataflow]:
        """Fetch the full BIS dataflow catalog. Results are cached."""
        if self._dataflow_cache is not None:
            return list(self._dataflow_cache)

        url = f"{self.BASE_URL}/structure/dataflow/BIS"
        response = self._get(url, headers={"Accept": "application/json"})
        data = response.json()

        dataflows: list[BISDataflow] = []
        for df_node in data.get("data", {}).get("dataflows", []):
            ...
            if isinstance(struct_val, str) and "DataStructure=" in struct_val:
                ...
                structure_id = cl_id
                # Extract version from URN: ...BIS:BIS_CBPOL(1.0)
    ...
    def get_datastructure(
        ...
        url = f"{self.BASE_URL}/structure/dataflow/BIS/{dataflow_id}/{df_version}"
        ...
        if not structures:
            raise BISAPIError(f"No data structure found for dataflow {dataflow_id}/{df_version}")
        ...
        for dim_node in dim_list.get("dimensions", []):
            ...
            dimensions.append(BISDimension(
                id=dim_id,
                position=position,
                name=str(name_val),
    ...
    def estimate_size(
        ...
        Uses a CSV probe with ``key=.`` (all series) and ``lastNObservations=1``.
        ...
        try:
            url = f"{self.BASE_URL}/data/dataflow/BIS/{dataflow_id}/{version}/."
            params: dict[str, str] = {
                "detail": "dataonly",
                "format": "csvdata",
                "lastNObservations": "1",
            ...
            # CSV rows minus header = observation count; each row is one series × 1 period
    ...
    def fetch_dataset_chunked(
        ...
        *,
        ...
        limit: int = 0,
        on_chunk: Callable[[list[BISObservation], str, str], None] | None = None,
    ) -> list[BISObservation]:
        ...
        all_obs: list[BISObservation] = []
        for start_period, end_period in chunk_ranges:
            logger.info(
                "BIS chunked fetch %s [%s – %s]",
