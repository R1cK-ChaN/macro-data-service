"""Drive Ifo schedule and value ingestion."""
...
from .indicators import INDICATOR_REGISTRY, IFO_SURVEY_URL, reference_label_en
from .parser import (
    PROVIDER,
...
class FetchRunSummary:
    """Outcome of one ``fetch_ifo_calendar`` invocation."""
...
class ScheduleRunSummary:
    """Outcome of one ``schedule_ifo_calendar`` invocation."""
...
def schedule_ifo_calendar(
    ...
    *,
    ...
    """Fetch Ifo release-date rows for whitelisted indicators."""
    ...
    if unknown:
        logger.warning("Ifo schedule fetch: unknown series skipped: %s", unknown)
    ...
    try:
        ...
        logger.warning("Ifo release-date fetch failed: %s", exc)
    ...
    raw_records: list[IfoCalendarRawRecord] = []
    event_records: list[IfoCalendarEventRecord] = []
...
def fetch_ifo_calendar(
    ...
    *,
    ...
    """Fetch due Ifo press-release pages and project value rows."""
    started = time.monotonic()
    known, unknown = _resolve_series(series_ids)
    ...
    if unknown:
        logger.warning("Ifo value fetch: unknown series skipped: %s", unknown)
    if dry_run or not known:
        summary.wall_seconds = time.monotonic() - started
        return summary
    ...
    for row in pending:
        ...
        try:
            ...
            logger.warning("Ifo value fetch failed for %s: %s", sid, exc)
