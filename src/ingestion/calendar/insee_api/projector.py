"""INSEE calendar projection helpers."""
...
def project_schedule_events(
    ...
    INSEE's agenda payload links to the generic publication calendar.
    Once the value-side fetch resolves a concrete press-release page,
    recurring schedule refreshes should preserve that release URL.
    """
    rows = list(records)
    ...
    for record in rows:
        existing = connection.execute(
            ...
            WHERE provider = ?
              ...
              AND source_url LIKE '%/en/statistiques/%'
            """,
