...
``"eurostat"`` / ``"destatis"`` / ``"zew"`` / ``"ifo"`` / ``"insee"`` / ``"ine"`` / ``"istat"`` / ``"federal-reserve"`` / ``"nbs"``) and reports per-indicator match coverage. Used to verify
...
OFFICIAL_PROVIDERS: tuple[str, ...] = (
    ...
    "nar", "ecb", "eurostat", "destatis", "zew", "ifo", "insee", "ine", "istat", "federal-reserve", "nbs", "mof-jp", "cao",
    "meti", "stat-bureau-jp",
)
