...
:mod:`ingestion.calendar.bea_api.projector`, and
:mod:`ingestion.calendar.ecb_api.projector`. With Fed landing, we now
have four concrete callers of the "merge-rule" projector variant.
The promotion into ``_official_shared`` stays deferred — the BLS P3
follow-up planned it once the third concrete caller existed; this
slice adds the fourth but promotion is a separate subtraction commit,
kept off P4's critical path so the scaffold ships in one reviewable
diff.
"""
...
from .parser import FedCalendarEventRecord, FedCalendarRawRecord
...
def store_raw(
    ...
    records: Iterable[FedCalendarRawRecord],
) -> int:
...
def project_events(
    connection: sqlite3.Connection,
    records: Iterable[FedCalendarEventRecord],
) -> int:
    """API-side upsert to ``cal_econ_event``; returns inserts + updates count.

    Cross-source merge rule (same as BLS + BEA + ECB): if the existing
    row already carries a ``datetime``-precision ``event_time_utc``,
    a later write with ``approximate`` precision does **not** clobber
    it. The Fed scaffold always writes ``datetime`` precision (14:00 ET
    is the published convention), so the CASE is a no-op on the
    schedule → schedule path — it's kept here so future dual-write
    slices (value-side scrapes, schedule rescrapes) merge cleanly.
    Value columns obey the ``observed_at`` monotonicity guard so a
