"""Drive Spain INE schedule and value ingestion."""
...
class FetchRunSummary:
    """Outcome of one ``fetch_ine_calendar`` invocation."""
...
class ScheduleRunSummary:
    """Outcome of one ``schedule_ine_calendar`` invocation."""
...
def schedule_ine_calendar(
    ...
    *,
    ...
    """Fetch INE publication-calendar rows for whitelisted indicators."""
    ...
    if unknown:
        logger.warning("INE schedule fetch: unknown series skipped: %s", unknown)
    ...
    try:
        payload = (
            ...
            else fetch_calendar_html(session=session)
        )
        ...
        logger.warning("INE calendar fetch failed: %s", exc)
    ...
    raw_records: list[INECalendarRawRecord] = []
    event_records: list[INECalendarEventRecord] = []
...
def fetch_ine_calendar(
    ...
    *,
    ...
    """Fetch due INE advance press releases and project value rows."""
    started = time.monotonic()
    known, unknown = _resolve_series(series_ids)
    ...
    if unknown:
        logger.warning("INE value fetch: unknown series skipped: %s", unknown)
    if dry_run or not known:
        summary.wall_seconds = time.monotonic() - started
        return summary
    ...
    for row in pending:
        ...
        try:
            ...
            logger.warning("INE value fetch failed for %s: %s", sid, exc)
