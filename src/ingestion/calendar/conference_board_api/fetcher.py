"""Drive Conference Board schedule and value ingestion."""
...
class FetchRunSummary:
    """Outcome of a single ``fetch_conference_board_calendar`` invocation."""
...
class ScheduleRunSummary:
    """Outcome of a single ``schedule_conference_board_calendar`` invocation."""
...
def _default_schedule_window() -> tuple[int, int]:
    today = datetime.now(timezone.utc).date()
    return _epoch_ms(today - timedelta(days=45)), _epoch_ms(today + timedelta(days=180))


def schedule_conference_board_calendar(
    ...
    *,
    ...
    json_fetcher=fetch_calendar_json,
) -> ScheduleRunSummary:
    """Fetch Conference Board release-calendar rows for whitelisted series."""
    started = time.monotonic()
    known, unknown = _resolve_series(series_ids)
    default_from, default_to = _default_schedule_window()
    resolved_from = from_epoch_ms if from_epoch_ms is not None else default_from
    resolved_to = to_epoch_ms if to_epoch_ms is not None else default_to
    summary = ScheduleRunSummary(
        series_planned=list(known),
        series_unknown=list(unknown),
        dry_run=dry_run,
        from_epoch_ms=resolved_from,
        to_epoch_ms=resolved_to,
    )
    if unknown:
        logger.warning(
            "Conference Board schedule fetch: unknown series skipped: %s",
            unknown,
        )
    ...
    try:
        ...
        row_issues: list[str] = []
        entries = parse_calendar_events_json(
            payload,
            series_ids=set(known),
            row_issues=row_issues,
        )
        summary.row_issues = row_issues
    except Exception as exc:
        logger.warning("Conference Board schedule fetch failed: %s", exc)
        summary.fetch_error = str(exc)
        summary.wall_seconds = time.monotonic() - started
        return summary
    if not entries:
        summary.series_empty.extend(known)
        summary.fetch_error = "no Conference Board schedule entries parsed"
        summary.wall_seconds = time.monotonic() - started
        return summary

    hits: dict[str, int] = {sid: 0 for sid in known}
    raw_records: list[ConferenceBoardCalendarRawRecord] = []
    event_records: list[ConferenceBoardCalendarEventRecord] = []
...
def fetch_conference_board_calendar(
    connection: sqlite3.Connection,
    *,
    series_ids: Iterable[str] | None = None,
    dry_run: bool = True,
    session: requests.Session | None = None,
    snapshot_epoch_ms: int | None = None,
    current_html_fetcher=fetch_indicator_html,
) -> FetchRunSummary:
    ...
    if unknown:
        logger.warning(
            "Conference Board value fetch: unknown series skipped: %s",
            unknown,
        )
    ...
    for sid in value_known:
        ...
        try:
            ...
            logger.warning("Conference Board value fetch failed for %s: %s", sid, exc)
    ...
    for sid in value_known:
        if sid not in summary.series_ok and all(sid != fail[0] for fail in summary.series_failed):
            summary.series_empty.append(sid)
    if not event_records:
        summary.fetch_error = "no Conference Board current values parsed"
        summary.wall_seconds = time.monotonic() - started
        return summary
