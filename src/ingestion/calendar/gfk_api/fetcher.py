"""Drive GfK / NIM Consumer Climate schedule and value ingestion."""
...
from .indicators import INDICATOR_REGISTRY, GFK_CONSUMER_CLIMATE_URL, reference_label_en
from .parser import (
    PROVIDER,
...
class FetchRunSummary:
    """Outcome of one ``fetch_gfk_calendar`` invocation."""
...
class ScheduleRunSummary:
    """Outcome of one ``schedule_gfk_calendar`` invocation."""
...
def schedule_gfk_calendar(
    ...
    *,
    ...
    """Fetch GfK / NIM release-date rows for whitelisted indicators."""
    ...
    if unknown:
        logger.warning("GfK schedule fetch: unknown series skipped: %s", unknown)
    ...
    try:
        ...
        logger.warning("GfK release-date fetch failed: %s", exc)
    ...
    raw_records: list[GfKCalendarRawRecord] = []
    event_records: list[GfKCalendarEventRecord] = []
...
def fetch_gfk_calendar(
    ...
    *,
    ...
    """Fetch due GfK / NIM press-release pages and project value rows."""
    started = time.monotonic()
    known, unknown = _resolve_series(series_ids)
    ...
    if unknown:
        logger.warning("GfK value fetch: unknown series skipped: %s", unknown)
    if dry_run or not known:
        summary.wall_seconds = time.monotonic() - started
        return summary
    ...
    for row in pending:
        ...
        try:
            listing_payload = (
                ...
                else fetch_all_releases_html(session=session)
            )
            ...
            logger.warning("GfK value fetch failed for %s: %s", sid, exc)
