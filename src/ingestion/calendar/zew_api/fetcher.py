"""Drive ZEW schedule and value ingestion."""
...
from .indicators import INDICATOR_REGISTRY, reference_label_en, release_dates_url
from .parser import (
    PROVIDER,
...
class FetchRunSummary:
    """Outcome of one ``fetch_zew_calendar`` invocation."""
...
class ScheduleRunSummary:
    """Outcome of one ``schedule_zew_calendar`` invocation."""
...
def schedule_zew_calendar(
    ...
    *,
    ...
    html_fetcher: Callable[[int], str | bytes] | None = None,
) -> ScheduleRunSummary:
    """Fetch ZEW release-date rows for whitelisted indicators."""
    ...
    if unknown:
        logger.warning("ZEW schedule fetch: unknown series skipped: %s", unknown)
    ...
    try:
        for year in range(resolved_start.year, resolved_end.year + 1):
            payload = (
                ...
                else fetch_release_dates_html(year, session=session)
            )
            entries.extend(
                parse_release_dates_html(
                    payload,
                    series_ids=set(known),
                    source_url=release_dates_url(year),
                    row_issues=summary.row_issues,
                )
            )
    except Exception as exc:
        logger.warning("ZEW release-date fetch failed: %s", exc)
    ...
    raw_records: list[ZEWCalendarRawRecord] = []
    event_records: list[ZEWCalendarEventRecord] = []
...
def fetch_zew_calendar(
    ...
    *,
    ...
    session: requests.Session | None = None,
    snapshot_epoch_ms: int | None = None,
    listing_fetcher: Callable[[date], str | bytes] | None = None,
    html_fetcher: Callable[[str], str | bytes] | None = None,
    now_utc: datetime | None = None,
) -> FetchRunSummary:
    """Fetch due ZEW press-release pages and project value rows."""
    started = time.monotonic()
    known, unknown = _resolve_series(series_ids)
    ...
    if unknown:
        logger.warning("ZEW value fetch: unknown series skipped: %s", unknown)
    if dry_run or not known:
        summary.wall_seconds = time.monotonic() - started
        return summary
    ...
    for row in pending:
        ...
        try:
            listing_payload = (
                listing_fetcher(release_date)
                if listing_fetcher is not None
                else fetch_latest_press_releases_html(session=session)
            )
            ...
            logger.warning("ZEW value fetch failed for %s: %s", sid, exc)
