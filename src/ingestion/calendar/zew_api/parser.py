"""Project ZEW press-release observations into calendar records."""
...
from .indicators import INDICATOR_REGISTRY, ZEWIndicatorSpec

PROVIDER = "zew"


@dataclass(frozen=True)
class ZEWValueObservation:
    """One headline value parsed from a ZEW press release."""
...
class ZEWPressReleaseParseError(ValueError):
    """Raised when a ZEW press-release page cannot be projected."""
...
_VALUE_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(
        rf"zew indicator of economic sentiment stands at\s+"
        rf"(?:(plus|minus)\s+)?{_NUMBER_RE}\s+points",
        re.I | re.S,
    ),
    re.compile(
        rf"at\s+(?:(plus|minus)\s+)?{_NUMBER_RE}\s+points",
        re.I | re.S,
    ),
)
...
def _value_text(raw: str, sign_word: str | None) -> str:
    cleaned = str(raw or "").strip().replace("+", "").replace(" ", "")
    cleaned = cleaned.replace(",", ".")
    try:
        Decimal(cleaned)
    except InvalidOperation as exc:
        raise ZEWPressReleaseParseError(f"invalid ZEW value: {raw!r}") from exc
    if sign_word == "minus" and not cleaned.startswith("-"):
        return f"-{cleaned}"
    return cleaned


def _extract_value(text: str, spec: ZEWIndicatorSpec) -> str:
    if spec.release_kind != "economic_sentiment":
        raise KeyError(f"unknown ZEW release kind: {spec.release_kind!r}")
    for pattern in _VALUE_PATTERNS:
        match = pattern.search(text)
        if match:
            sign_word, raw_value = match.groups()
            return _value_text(raw_value, sign_word)
    raise ZEWPressReleaseParseError(
...
def parse_press_release_value(
    ...
    *,
    ...
    """Extract the ZEW economic-sentiment headline value."""
