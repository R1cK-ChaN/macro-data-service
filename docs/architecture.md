...
- **P-sched-1 (shipped — schedule-side recurring refresh driver):** `src/ingestion/calendar/scheduler.py` exposes `refresh_all_schedules(connection_factory, dry_run=True, connectors=?)` — the daily-cron entry point that iterates every connector's forward-looking schedule scrape (`schedule_bls_calendar` / `schedule_bea_calendar` / `schedule_ecb_calendar` / `fetch_fed_calendar` / `fetch_fed_releasedates` / `fetch_nbs_calendar`) in sequence. Each connector gets its own connection / commit / rollback lifecycle so an ECB 502 or NBS timeout rolls back only that connector's partial writes while the others stay committed; per-connector exceptions land on the returned `RefreshRunSummary.results` as `ConnectorResult(ok=False, error=...)` rather than aborting the pass. Subset argument (`connectors=["bls", "fed-fomc"]`) lets an operator re-run only specific sources after triage. `_connector_overrides` is a test-only seam so unit tests can exercise the aggregation + isolation logic against fake functions. Driver also inspects the returned summary for in-summary failure markers — BEA's `fetch_error`, Fed releasedates' `fetch_error`, ECB's `fetch_errors` dict, BLS's `series_failed` list — and flips `ok=False` with the recorded reason so a 502 / layout drift caught by the connector (rather than raised) doesn't masquerade as a clean run from the top-level envelope. Unknown connector names in the `connectors=[...]` subset (``"fed-fomcc"`` / other operator typos) land on `RefreshRunSummary.unknown_connectors` and count toward `failed_count`, so a cron-level typo surfaces loudly rather than silently skipping the intended source. Service op `calendar_econ_refresh_schedules(dry_run=True, connectors=?)` exposes the driver; 14 new tests, no real upstream HTTP in CI. Value-side sweep (triggered when an event crosses its scheduled time so `actual` fills automatically), budget guards / circuit breakers, and health-telemetry wiring into `get_source_health_dashboard` are follow-up P-sched slices.
- **Remaining:** per-connector `-live` slices (BEA / ECB / Fed / NBS operator-driven `--execute` runs + reports); indicator-whitelist expansion on the `-live` slices (BEA PCE / Trade Balance / Corporate Profits; NBS PPI / GDP / IndProd / FAI / Retail Sales / Mfg + Non-mfg PMI); **P-sched** follow-ups (value-side sweep, budget guards, health telemetry — P-sched-1 shipped the schedule-side aggregator); P6 cross-provider parity harness (needs real official-source rows to compare against TE); P8 TE subscription retirement once P6 passes.
