...
- **Remaining:** full 2013→today TE backfill execution; `/calendar/dividends` JSON:API pagination (currently silently truncates at 1000); official-source economic connectors (BLS/ECB/Fed/NBS); legacy HTML-scraped `calendar_events` parity + retirement; downstream `GET /calendar` HTTP surface over `v_calendar_item`.
