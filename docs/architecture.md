...
- **P-sched-4 (shipped — health telemetry wiring):** `SourceCapabilityManager.get_customer_health()` now calls a new `_calendar_connectors_block()` helper that reads every row from `calendar_connector_state` via `scheduler_state.list_connector_states(conn)` and classifies each as `ok` / `failing` (pre-threshold) / `cooling` / `budget_exhausted`. The dashboard dict grew a top-level `calendar_connectors: [...]` array plus three summary counters (`calendar_cooling` / `calendar_budget_exhausted` / `calendar_failing`); any cooling or budget-exhausted connector flips the top-level `status` to `degraded` so `/health` consumers notice a stale forward calendar without parsing the nested block. Early-warning `failing` (partial consecutive failures below the cool-down threshold) does NOT flip the top-level status — it's surfaced in the summary counter only so alerting stays anchored on the breaker / budget trip, not transient flakiness. The block's read is exception-safe: a missing migration or empty DB returns an empty block rather than 500'ing the health endpoint. 5 new tests on `SourceCapabilityManager.get_customer_health` covering empty / cooling / budget-exhausted / failing / ok-with-row paths.
- **Remaining:** per-connector `-live` slices (BEA / ECB / Fed / NBS operator-driven `--execute` runs + reports); indicator-whitelist expansion on the `-live` slices (BEA PCE / Trade Balance / Corporate Profits; NBS PPI / GDP / IndProd / FAI / Retail Sales / Mfg + Non-mfg PMI); P6 cross-provider parity harness (needs real official-source rows to compare against TE); P8 TE subscription retirement once P6 passes.
