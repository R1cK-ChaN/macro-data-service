...
- **Remaining:** per-connector `-live` slices (BEA / ECB / Fed / NBS operator-driven `--execute` runs + reports); indicator-whitelist expansion on the `-live` slices (BEA PCE / Trade Balance / Corporate Profits; NBS PPI / GDP / IndProd / FAI / Retail Sales / Mfg + Non-mfg PMI); **P-sched** recurring fetch scheduler (issue-body target end-state #1 — "within minutes of publication" — is blocked until this lands); P6 cross-provider parity harness (needs real official-source rows to compare against TE); P8 TE subscription retirement once P6 passes.
