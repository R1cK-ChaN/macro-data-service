...
- **Remaining:** indicator-whitelist expansion (BLS / BEA / ECB / Fed); schedule scrapers per source (BLS P1a shipped; BEA P2a + ECB P3a + Fed P4a + NBS P5a pending); P5 NBS connector; `_official_shared` projector promotion (now-overdue subtraction pass); P6 cross-provider parity harness; P7 downstream `GET /calendar` HTTP surface over `v_calendar_item`; P8 TE subscription retirement once P6 passes.
